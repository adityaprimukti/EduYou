import { useMemo } from "react";
import "./MentorSearchContainer.css";
import { Link } from "react-router-dom";

const MentorSearchContainer = ({ frameTop, frameLeft }) => {
  const temukanMentorStyle = useMemo(() => {
    return {
      top: frameTop,
      left: frameLeft,
    };
  }, [frameTop, frameLeft]);

  return (
    <div className="temukanmentor1" style={temukanMentorStyle}>
      <div className="frame7">
        <div className="ellipse-parent2">
          <div className="frame-child26" />
          <b className="temukan-mentormu">
            <span>{`Temukan `}</span>
            <span className="mentormu">Mentormu</span>
          </b>
        </div>
      </div>
      <div className="frame8">
        <img className="frame-icon1" alt="" src="/frame.svg" />
        <div className="frame9">
          <div className="write-your-keywoard1">Write your keywoard</div>
        </div>
      </div>
      <div className="frame-parent6">
        <div className="frame-parent7">
          <div className="frame-wrapper1">
            <div className="frame-parent8">
              <div className="rectangle-wrapper">
                <div className="frame-child27" />
              </div>
              <div className="ellipse-parent3">
                <div className="frame-child28" />
                <div className="frame-child29" />
                <div className="frame-child30" />
              </div>
            </div>
          </div>
          <div className="frame-wrapper2">
            <div className="frame-wrapper3">
              <div className="frame-parent9">
                <button className="frame-wrapper4">
                  <div className="mask-group-parent">
                    <img
                      className="mask-group-icon2"
                      alt=""
                      src="/mask-group@2x.png"
                    />
                    <div className="frame-wrapper5">
                      <div className="frame-parent10">
                        <div className="rectangle-parent3">
                          <div className="frame-child31" />
                          <div className="frame-child32" />
                          <div className="frame-child33" />
                        </div>
                        <div className="adam-levin">Adam Levin</div>
                      </div>
                    </div>
                  </div>
                </button>
                <div className="frame-wrapper6">
                  <div className="frame-parent11">
                    <div className="frame-wrapper6">
                      <div className="frame-parent12">
                        <div className="star-parent">
                          <img className="star-icon" alt="" src="/star-8.svg" />
                          <div className="ulasan2">
                            <b>{`5 `}</b>
                            <span>(200 ulasan)</span>
                          </div>
                        </div>
                        <div className="germany-learn-english-container">
                          <b>Germany</b>
                          <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                        </div>
                      </div>
                    </div>
                    <div className="rp-10000000-parent">
                      <b className="rp-10000000">Rp 100.000,00</b>
                      <b className="kursus-pertama-gratis">
                        Kursus pertama gratis
                      </b>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="frame-parent7">
          <div className="frame-wrapper1">
            <div className="frame-parent8">
              <div className="rectangle-wrapper">
                <div className="frame-child27" />
              </div>
              <div className="frame10">
                <div className="ellipse-parent4">
                  <div className="frame-child28" />
                  <div className="frame-child29" />
                  <div className="frame-child30" />
                </div>
              </div>
            </div>
          </div>
          <div className="frame-wrapper2">
            <div className="frame-wrapper3">
              <div className="frame-parent9">
                <div className="frame-wrapper11">
                  <button className="mask-group-group">
                    <img
                      className="mask-group-icon2"
                      alt=""
                      src="/mask-group1@2x.png"
                    />
                    <div className="frame-wrapper5">
                      <div className="frame-parent10">
                        <div className="rectangle-parent3">
                          <div className="frame-child31" />
                          <div className="frame-child32" />
                          <div className="frame-child33" />
                        </div>
                        <div className="adam-levin">Klara Troye</div>
                      </div>
                    </div>
                  </button>
                </div>
                <div className="frame-wrapper6">
                  <div className="frame-parent11">
                    <div className="frame-wrapper6">
                      <div className="frame-parent12">
                        <div className="star-parent">
                          <img className="star-icon" alt="" src="/star-8.svg" />
                          <div className="ulasan2">
                            <b>{`5 `}</b>
                            <span>(200 ulasan)</span>
                          </div>
                        </div>
                        <div className="germany-learn-english-container">
                          <b>Germany</b>
                          <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                        </div>
                      </div>
                    </div>
                    <div className="rp-10000000-parent">
                      <b className="rp-10000000">Rp 100.000,00</b>
                      <b className="kursus-pertama-gratis">
                        Kursus pertama gratis
                      </b>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="frame-parent7">
          <div className="frame-wrapper1">
            <div className="frame-parent8">
              <div className="rectangle-wrapper">
                <div className="frame-child27" />
              </div>
              <div className="frame10">
                <div className="ellipse-parent4">
                  <div className="frame-child28" />
                  <div className="frame-child29" />
                  <div className="frame-child30" />
                </div>
              </div>
            </div>
          </div>
          <div className="frame-wrapper2">
            <div className="frame-wrapper3">
              <div className="frame-parent9">
                <button className="frame-wrapper4">
                  <div className="mask-group-parent">
                    <img
                      className="mask-group-icon2"
                      alt=""
                      src="/mask-group2@2x.png"
                    />
                    <div className="frame-wrapper19">
                      <div className="frame-parent10">
                        <div className="rectangle-parent3">
                          <div className="frame-child31" />
                          <div className="frame-child32" />
                          <div className="frame-child33" />
                        </div>
                        <div className="tiara-victory">Tiara Victory</div>
                      </div>
                    </div>
                  </div>
                </button>
                <div className="frame-wrapper6">
                  <div className="frame-parent11">
                    <div className="frame-wrapper6">
                      <div className="frame-parent12">
                        <div className="star-parent">
                          <img className="star-icon" alt="" src="/star-8.svg" />
                          <div className="ulasan2">
                            <b>{`5 `}</b>
                            <span>(200 ulasan)</span>
                          </div>
                        </div>
                        <div className="germany-learn-english-container">
                          <b>Germany</b>
                          <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                        </div>
                      </div>
                    </div>
                    <div className="rp-10000000-parent">
                      <b className="rp-10000000">Rp 100.000,00</b>
                      <b className="kursus-pertama-gratis">
                        Kursus pertama gratis
                      </b>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="temukanmentor-inner">
        <div className="frame-wrapper22">
          <div className="frame-parent25">
            <div className="frame-parent7">
              <div className="frame-wrapper1">
                <div className="frame-parent8">
                  <div className="rectangle-wrapper">
                    <div className="frame-child27" />
                  </div>
                  <div className="ellipse-parent3">
                    <div className="frame-child28" />
                    <div className="frame-child29" />
                    <div className="frame-child30" />
                  </div>
                </div>
              </div>
              <div className="frame-wrapper2">
                <div className="frame-wrapper3">
                  <div className="frame-parent9">
                    <button className="frame-wrapper4">
                      <div className="mask-group-parent">
                        <img
                          className="mask-group-icon2"
                          alt=""
                          src="/mask-group3@2x.png"
                        />
                        <div className="frame-wrapper5">
                          <div className="frame-parent10">
                            <div className="rectangle-parent3">
                              <div className="frame-child31" />
                              <div className="frame-child32" />
                              <div className="frame-child33" />
                            </div>
                            <div className="adam-levin">Adam Levin</div>
                          </div>
                        </div>
                      </div>
                    </button>
                    <div className="frame-wrapper6">
                      <div className="frame-parent11">
                        <div className="frame-wrapper6">
                          <div className="frame-parent12">
                            <div className="star-parent">
                              <img
                                className="star-icon"
                                alt=""
                                src="/star-8.svg"
                              />
                              <div className="ulasan2">
                                <b>{`5 `}</b>
                                <span>(200 ulasan)</span>
                              </div>
                            </div>
                            <div className="germany-learn-english-container">
                              <b>Germany</b>
                              <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                            </div>
                          </div>
                        </div>
                        <div className="rp-10000000-parent">
                          <b className="rp-10000000">Rp 100.000,00</b>
                          <b className="kursus-pertama-gratis">
                            Kursus pertama gratis
                          </b>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="frame-parent7">
              <div className="frame-wrapper1">
                <div className="frame-parent8">
                  <div className="rectangle-wrapper">
                    <div className="frame-child27" />
                  </div>
                  <div className="frame10">
                    <div className="ellipse-parent4">
                      <div className="frame-child28" />
                      <div className="frame-child29" />
                      <div className="frame-child30" />
                    </div>
                  </div>
                </div>
              </div>
              <div className="frame-wrapper2">
                <div className="frame-wrapper3">
                  <div className="frame-parent9">
                    <button className="frame-wrapper4">
                      <div className="mask-group-parent">
                        <img
                          className="mask-group-icon2"
                          alt=""
                          src="/mask-group4@2x.png"
                        />
                        <div className="frame-wrapper5">
                          <div className="frame-parent10">
                            <div className="rectangle-parent3">
                              <div className="frame-child31" />
                              <div className="frame-child32" />
                              <div className="frame-child33" />
                            </div>
                            <div className="adam-levin">Klara Troye</div>
                          </div>
                        </div>
                      </div>
                    </button>
                    <div className="frame-wrapper6">
                      <div className="frame-parent11">
                        <div className="frame-wrapper6">
                          <div className="frame-parent12">
                            <div className="star-parent">
                              <img
                                className="star-icon"
                                alt=""
                                src="/star-8.svg"
                              />
                              <div className="ulasan2">
                                <b>{`5 `}</b>
                                <span>(200 ulasan)</span>
                              </div>
                            </div>
                            <div className="germany-learn-english-container">
                              <b>Germany</b>
                              <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                            </div>
                          </div>
                        </div>
                        <div className="rp-10000000-parent">
                          <b className="rp-10000000">Rp 100.000,00</b>
                          <b className="kursus-pertama-gratis">
                            Kursus pertama gratis
                          </b>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="frame-parent7">
              <div className="frame-wrapper1">
                <div className="frame-parent8">
                  <div className="rectangle-wrapper">
                    <div className="frame-child27" />
                  </div>
                  <div className="ellipse-parent3">
                    <div className="frame-child28" />
                    <div className="frame-child29" />
                    <div className="frame-child30" />
                  </div>
                </div>
              </div>
              <div className="frame-wrapper2">
                <div className="frame-wrapper3">
                  <div className="frame-parent9">
                    <div className="frame-wrapper11">
                      <button className="mask-group-group">
                        <img
                          className="mask-group-icon2"
                          alt=""
                          src="/mask-group5@2x.png"
                        />
                        <div className="frame-wrapper19">
                          <div className="frame-parent10">
                            <div className="rectangle-parent3">
                              <div className="frame-child31" />
                              <div className="frame-child32" />
                              <div className="frame-child33" />
                            </div>
                            <div className="tiara-victory">Tiara Victory</div>
                          </div>
                        </div>
                      </button>
                    </div>
                    <div className="frame-wrapper6">
                      <div className="frame-parent11">
                        <div className="frame-wrapper6">
                          <div className="frame-parent12">
                            <div className="star-parent">
                              <img
                                className="star-icon"
                                alt=""
                                src="/star-8.svg"
                              />
                              <div className="ulasan2">
                                <b>{`5 `}</b>
                                <span>(200 ulasan)</span>
                              </div>
                            </div>
                            <div className="germany-learn-english-container">
                              <b>Germany</b>
                              <span>{`- learn english from a native speaker in Bandung who won multiple... `}</span>
                            </div>
                          </div>
                        </div>
                        <div className="rp-10000000-parent">
                          <b className="rp-10000000">Rp 100.000,00</b>
                          <b className="kursus-pertama-gratis">
                            Kursus pertama gratis
                          </b>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <Link to="/temukanmentor">
      <button className="frame-parent44">
        <div className="frame-child74" />
        <div className="lihat-lebih-banyak6">Lihat lebih banyak</div>
      </button>
      </Link>
    </div>
  );
};

export default MentorSearchContainer;
